...
def candidate_arithmetic(public, ranking, *, heldout=None, truth=None, metrics=None):
    with localcontext() as ctx:
        ctx.prec = 80
        all_rows = public['train']+public['validation']+(heldout or [])
        logs = {v: dec(v).ln() for r in all_rows for v in [r['target'], *r['features'].values()]}
        rows = {}
        max_error = Decimal(0)
        for c in ranking:
            def residual(row, intercept):
                return intercept+sum(dec(int(e))*logs[row['features'][k]]
                                     for k, e in c['expanded'].items())-logs[row['target']]
