...
def direct(a, b):
    require(git('rev-list', '--parents', '-n', '1', b).split()[1:] == [a], 'stage parent chronology')
...
def source_check():
    ...
    require(set(pf) == {'started_utc', 'completed_utc', 'environment', 'source_manifest', 'command', 'returncode',
        'stdout_sha256', 'stderr_sha256', 'fixture_seed_label', 'qualified', 'numerical_corners', 'mutations',
        'exception', 'qualified_attempt', 'attempt_receipt_sha256', 'prior_attempts'}, 'preflight closed schema')
    attempt = PKG/pf['qualified_attempt']
    require(sha((attempt/'receipt.json').read_bytes()) == pf['attempt_receipt_sha256'], 'qualified preflight receipt')
    schema.same(loads((attempt/'receipt.json').read_bytes()), {k: v for k, v in pf.items()
        if k not in ('qualified_attempt', 'attempt_receipt_sha256', 'prior_attempts')}, 'qualification report derivation')
    for stream in ('stdout', 'stderr'):
        require(sha((attempt/('tests.'+stream)).read_bytes()) == pf[stream+'_sha256'], 'preflight test stream')
    require(pf['returncode'] == 0 and pf['exception'] is None and pf['numerical_corners']['pass'] is True,
            'preflight tests/numeric qualification')
    for path, h in pf['source_manifest'].items():
        require(sha((attempt/'source'/path).read_bytes()) == h, 'executed preflight source snapshot')
    for prior in pf['prior_attempts']:
        receipt_path = PKG/prior['path']/'receipt.json'
        require(sha(receipt_path.read_bytes()) == prior['receipt_sha256'], 'retained prior attempt receipt')
        prior_receipt = loads(receipt_path.read_bytes())
        require(prior_receipt['qualified'] is prior['qualified'], 'prior attempt outcome')
        for path, h in prior_receipt['source_manifest'].items():
            require(sha((receipt_path.parent/'source'/path).read_bytes()) == h, 'prior source retention')
        for stream in ('stdout', 'stderr'):
            require(sha((receipt_path.parent/('tests.'+stream)).read_bytes()) == prior_receipt[stream+'_sha256'], 'prior failure stream')
...
def launch_checks(source, stopped=None):
    stages = ['seed', 'data', 'calibration', 'discovery', 'evaluation']
    if stopped is not None: stages = stages[:stages.index(stopped)+1]
    require({p.name for p in (PKG/'execution_logs').iterdir()} == set(stages), 'exact executed stage inventory')
    predecessor = {'seed': None, 'data': 'seed_commitment.json', 'calibration': 'commitment.json',
                   'discovery': 'calibration_seal.json', 'evaluation': 'selection_seal.json'}
    last = None
    for stage in stages:
        path = f'execution_logs/{stage}/receipt.json'
        r = artifact(path, 'stage-launch')
        schema.validate(r, {'stage': str, 'parent_sha': schema.SHA, 'command': schema.seq(str),
            'started_utc': str, 'completed_utc': str, 'returncode': schema.INTEGER,
            'stdout_sha256': schema.HASH, 'stderr_sha256': schema.HASH})
        require(r['stage'] == stage and (r['returncode'] != 0 if stage == stopped else r['returncode'] == 0), 'stage outcome')
        require(r['parent_sha'] == (source if predecessor[stage] is None else creation(predecessor[stage])), 'stage input committed before execution')
        require(r['started_utc'] <= r['completed_utc'] and (last is None or last < r['started_utc']), 'stage wall-clock chronology')
        require(r['command'][1:5] == ['-B', '-m', 'Discovery.estimated_noise1', stage], 'executed stage command')
        for stream in ('stdout', 'stderr'):
            require(sha((PKG/f'execution_logs/{stage}/{stream}').read_bytes()) == r[stream+'_sha256'], 'stage output retention')
        last = r['completed_utc']


def check_failure(p, pf, source):
    from Discovery.estimated_noise1_failure import inputs, derive
    archive = artifact('failure_archive.json', 'failure-archive')
    schema.validate(archive, {'source_sha': schema.SHA, 'stopped_commit_sha': schema.SHA,
        'retained_files': schema.mapping(schema.HASH), 'completed_numeric_ids': schema.seq(str), 'failure_count': schema.INTEGER})
    require(archive['source_sha'] == source, 'stopped source epoch')
    ancestor(source, archive['stopped_commit_sha'])
    require(archive['failure_count'] == 1, 'stop at first scientific failure; no retries')
    for path, h in archive['retained_files'].items():
        require(sha((PKG/path).read_bytes()) == h and sha(blob(archive['stopped_commit_sha'], REL+'/'+path)) == h,
                'preserved partial evidence '+path)
        creation(path)
    failures, rows, numeric = inputs()
    require(len(failures) == archive['failure_count'] and sorted(numeric) == archive['completed_numeric_ids'], 'stopped inventory')
    f = failures[0]
    require(set(f) == {'stage', 'parent_sha', 'source_sha', 'observed_utc', 'classification', 'error_type', 'error',
        'traceback', 'worker_output', 'retention', 'scientific_pipeline_stopped', 'scientific_retry_authorized'}, 'failure schema')
    require(f['source_sha'] == source and f['error_type']+': '+f['error'] in f['traceback'], 'error attribution')
    launch_checks(source, f['stage'])
    if numeric:
        require(f['stage'] == 'evaluation', 'only post-seal evaluated records interpretable')
        ...
        cal = artifact('calibration.json', 'calibration')['datasets']
        com = artifact('commitment.json', 'commitment')
        for path in ('heldout.json', 'oracle_reveal.json'):
            require(sha((PKG/path).read_bytes()) == com['private_hashes'][path], 'partial reveal commitment')
        # These were already revealed before failure; no new evaluator access.
        for rid in numeric:
            sel = artifact(f'selection/{rid}.json', 'selection')
            schema.validate_selection(sel); receipt_check(sel['worker'], source, pf['source_manifest'])
            e = artifact('calibration_estimates.json', 'calibration-estimates')['records'][rid]['discovery']['estimates']
            truth = oracle['datasets'][rid]
            close(rows[rid], science.evaluate(p, sel, truth, heldout[rid], e))
            intended = [truth['epsilon']*(r['u1']-r['u2']) for r in oracle['blocks'][str(truth['block'])]['calibration']]
            reference.calibration(cal[rid], science.settings(p), e, intended)
            reference.candidate_arithmetic(public[rid], sel['worker']['discovery']['ranking'], heldout=heldout[rid],
                truth=truth, metrics={c['class_id']: c for c in rows[rid]['candidates']})
    summary = derive(p, failures, rows, numeric)
    schema.same(artifact('summary.json', 'summary'), summary, 'failure-aware summary derivation')
    result = artifact('result.json', 'raw-result'); schema.validate(result, schema.RESULT)
    expected = {'source_sha': source,
        'selection_seal_commit_sha': creation('selection_seal.json') if (PKG/'selection_seal.json').exists() else None,
        'summary_sha256': sha((PKG/'summary.json').read_bytes()),
        **{k+'_sha256': sha((PKG/(k+'.json')).read_bytes()) if (PKG/(k+'.json')).exists() else None for k in ('evaluation', 'controls')},
        **{k: summary[k] for k in ('disposition', 'evidence_integrity', 'interpretable_datasets', 'expected_datasets', 'review_status')},
        'operational_status': 'STOPPED_AT_FIRST_SCIENTIFIC_FAILURE', 'outcome_blind': False, 'scientific_law_claim': False}
    schema.same(result, expected, 'honest adverse raw result; no forced favorable label')
    direct(archive['stopped_commit_sha'], creation('result.json'))
    return {k: result[k] for k in ('disposition', 'evidence_integrity', 'interpretable_datasets', 'source_sha', 'review_status')}
...
def check(*, numeric=True):
    ...
    if (PKG/'failure_archive.json').exists(): return check_failure(p, pf, source)
