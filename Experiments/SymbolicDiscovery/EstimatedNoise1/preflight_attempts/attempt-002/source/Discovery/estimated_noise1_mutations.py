...
SPECS = {
    ...
    'M4': (calibration, 's, pairs = validate(payload)',
           "copied = dict(payload)\n    true_epsilon = copied.pop('true_epsilon', None)\n    s, pairs = validate(copied)\n    if true_epsilon is not None:\n        pairs = [{**r, 'response_1': math.exp(true_epsilon), 'response_2': 1.0} for r in pairs]", 'PAIR_ONLY_SCHEMA'),
