...
def numerical_corners():
    ...
    # Full generator/schema/reference exercise on the fixed engineering seed.
    # No detector outcomes are used here and no prospective master seed exists.
    generated = science.generate(p, 'e'*64)
    public, calibration, heldout, oracle = generated
    schema.scientific_inputs(public, calibration, heldout, oracle)
    generated_reference = reference.generation(p, public, calibration, heldout, oracle)
    return {'corners': len(reports), 'pass': all(r['pass'] for r in reports+alternate+candidate_reports) and generated_reference['pass'],
            ...
            'engineering_generator_reference': generated_reference}
