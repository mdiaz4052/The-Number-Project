...
PREFIX = 'tests.test_' + n.STEM + '_mutations.TorqueMutationBehaviorTests.'
...
CASES = (
    case('one_sided_factor', 'K = 16 * ms * mt * gamma / radius', 'K = 8 * ms * mt * gamma / radius', 'test_peak_difference', 0),
    case('metric_prefix', "rational(units['pNm_to_Nm']) /", "rational(units['nNm_to_Nm']) /", 'test_metric_prefix', 1),
    case('nominal_geometry', 'row = original', "row = source['campaigns'][0] if k < 3 else original", 'test_campaign_geometry', 2),
    case('excluded_dependency', 'response = b / K', "response = b / K + rational(p['excluded_context'].get('G_values', [0])[0])", 'test_excluded_invariance', 3),
    case('unlicensed_averaging', 'return None  # No unlicensed equal-weight substitution.',
         'return (sum(d[::2]) / 3, sum(d[1::2]) / 3)', 'test_unlicensed_aggregation', 4),
    case('erased_coupling', 'return response\n', 'return (response[0], F(0), *response[2:])\n', 'test_coupled_response', 5),
    case('subspace_equivalence', 'return mean_subspace(a) == mean_subspace(b)', 'return tuple(a) == tuple(b)', 'test_subspace_equivalence', 6),
    case('promoted_local', "if evidence['stage'] != 'final' or evidence['domain'] != 'global':", 'if False:', 'test_stage_promotion', 7),
    case('faulty_affine_calibration', 'return mv(H, d)', 'return tuple(-v for v in mv(H, d))', 'test_faulty_calibration'),
    case('equivalent_response_control', 'return response\n', 'return tuple(response)\n', 'test_equivalent_control', expected='SURVIVED'),
)
...
def validate_definitions(source, protocol):
    ...
    for i, (c, req) in enumerate(zip(production, protocol['mutation_requirements'])):
        if c['id'] != req['id'] or c['tests'] != [req['test']] or c['requirement_index'] != i:
...
def execute(root, c):
    with TemporaryDirectory(prefix='nist-torque-mutant-') as temp:
        target = Path(temp)
...
def main(argv=None):
    ...
    try:
        if args.check:
            check_artifact();print('NIST torque mutation evidence verified: 8/8; valid controls')
        else:
